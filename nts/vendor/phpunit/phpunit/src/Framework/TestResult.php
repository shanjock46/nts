...
final class TestResult implements Countable
{
    ...
    public static function isAnyCoverageRequired(TestCase $test)
    {
        ...
        // If there are no explicit covers, and the tests class is
        // marked as covers nothing, all coverage can be skipped
        if (isset($annotations['class']['coversNothing'])) {
            return false;
        }

        // Otherwise each tests method can generate coverage
        return true;
    }
            ...
            try {
                ...
                $this->addFailure(
                    ...
                    new CoveredCodeNotExecutedException(
                        'This tests did not execute all the code that is listed as code to be covered:' .
                        \PHP_EOL . $cce->getMessage()
                    ),
                    $time
                );
